...
def make_move(state):
    ...
    if state.round == 1:
        dot_idx = np.where(field_state[5] != '.')[0]
        r1_tokens = np.array(field_state[5][dot_idx]).astype(float)
    ...
    if np.sum(current_board[state.round]) == -1:
        me_idx = np.where(current_board[state.round] == 1)
        enemy_idx = np.where(current_board[state.round] == -1)

    # If sum = 0, Even # Tokens & Yellow moved first
    elif np.sum(current_board[state.round]) == 0 and state.first == 'Yellow':
        me_idx = np.where(current_board[state.round] == 1)
        enemy_idx = np.where(current_board[state.round] == -1)

    # If sum = 0, Even # Tokens & Red moved first
    elif np.sum(current_board[state.round]) == 0:
        me_idx = np.where(current_board[state.round] == -1)
        enemy_idx = np.where(current_board[state.round] == 1)

    # Create me board
    me_board = np.zeros_like(current_board[state.round])
    me_board[me_idx] = 1
    me_board[enemy_idx] = -1

    # Check if any columns are winning columns:
